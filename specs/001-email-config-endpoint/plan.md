...
- **Scaffolding & Starters**: The service was originally generated via Spring Initializr (https://start.spring.io with dependencies `web,validation,data-firestore`) and any new module wiring for this feature (controllers, services, config) must continue to rely on official Spring Boot Starters per Constitution v2.2.0 Runtime Stack guardrail; no ad-hoc manual bootstrapping is allowed.
...
- **Storage**: Single Firestore document `appSettings/configuration/delivery/main` holds the fields listed in the spec. Postgres email audit log schema is untouched.
...
- **Persistent settings**: `appSettings/configuration/delivery/main` schema, defaults, migration steps, cache TTL, and rollback instructions match the spec and are reiterated in `data-model.md`. Always Free quotas are respected with low write volume and emulator-based tests.

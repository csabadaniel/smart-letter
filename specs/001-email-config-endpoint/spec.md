...
- **Scaffolding prerequisite**: If the repository ever loses its baseline skeleton (e.g., new environment, clean-room rebuild), teams MUST re-run Spring Initializr before Phase 1 tasks begin using a documented command such as `curl https://start.spring.io/starter.zip -d dependencies=web,validation,data-firestore,actuator -d javaVersion=21 -d language=java -d type=maven-project -d packageName=com.smartletter -o smart-letter.zip`, extract it, and commit the generated structure before continuing. This satisfies the Constitution requirement that the project be created via Spring Initializr exactly once at the start of implementation.

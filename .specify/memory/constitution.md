...
Version: 2.3.0 -> 2.4.0
Modified Principles:
- Service Guardrails – Continuous Delivery Pipelines (SemVer tagged production deploys)
- Workflow & Quality Gates – Continuous Delivery Evidence (SemVer tag documentation)
- Workflow & Quality Gates – Release Gate (SemVer tag proof in release notes)
Added Sections:
- None
Removed Sections:
- None
...
### I. Code-First Interfaces
- Treat annotated Spring MVC controllers and DTOs as the canonical source of truth. Each change must update the code annotations first and use the Springdoc/OpenAPI Maven plugin (e.g., `./mvnw springdoc-openapi:generate`) to emit `docs/contracts/openapi.yaml`; the generated file must not be hand-edited.
- Enforce `jakarta.validation`, explicit error contracts, and descriptive metadata (operation summaries, request/response examples, auth requirements) inside the code annotations so the generated OpenAPI + Swagger UI remain rich enough for humans and client SDKs.
- Introduce new request/response fields in a backward compatible way; breaking changes require versioned routes or header-based negotiation plus a documented migration window, and the generated OpenAPI diff must highlight the change for reviewers.
*Rationale*: Code-first governance keeps documentation and behavior perfectly aligned while still producing a publishable OpenAPI bundle for consumers and tooling.
...
- **GCP Deployment**: The service runs on Cloud Run (regional) configured to stay within the Always Free tier (<= 1 vCPU, <= 256 MiB memory, 2M requests/month). Set max concurrency <= 20 and keep CPU allocation `on-demand` to honor the cost envelope. The end-to-end request -> LLM -> email flow MUST meet a <5s p95 latency target (matching the plan template); exceeding that budget triggers alerts and remediation. Configure the Cloud Run request timeout to <= 60s only as a hard fail-safe so slow paths still terminate deterministically. Use Cloud Secret Manager for API keys and Cloud Logging/Monitoring for runtime telemetry. Persistent state must not rely on writable container filesystem; use GCP managed services instead.
...
- **API Key Access Control**: All inbound traffic MUST present an API key representing at least 32 bytes (256 bits) of cryptographic randomness - e.g., a base64-encoded 32-byte value - in the `X-SmartLetter-Api-Key` header. Keys are generated by the platform team, stored only in Cloud Secret Manager, rotated at least every 90 days, and scoped per environment. Authentication middleware must enforce constant-time comparison, per-key rate limits, and structured audit logs. Swagger UI Try-It-Out may send the API key via the same header once the user enters it through a secure input control - no keys baked into the frontend.
- **Generated OpenAPI Artifact**: The `docs/contracts/openapi.yaml` bundle must be produced directly from the annotated controllers via Springdoc/OpenAPI build steps in CI. Generated output may be committed for review but cannot be edited manually; reviewers rely on the diff plus Swagger UI to validate changes.
...
- **Continuous Delivery Pipelines**: Maintain separate **test** (staging) and **production** environments. GitHub Actions workflows must (1) on every commit to any feature branch run the full quality gate (lint, TDD, BDD, contracts, IaC validation, container build) and, when successful, deploy automatically to the test environment; (2) on merges into the protected release branch (e.g., `main`) rerun the gates and deploy to production. Deployments are blocked if any gate fails or if Firestore/Cloud Run quota checks would be exceeded. Every workflow publishes artifacts (JUnit, Cucumber, Terraform plan, container digest) and tags the deployed Cloud Run revision with the commit SHA. Production deployments MUST also create and push an annotated semantic version tag (`vMAJOR.MINOR.PATCH`) pointing at the deployed commit, include the workflow URL in the tag annotation, and record the version inside Cloud Run/Firestore metadata for traceability.

## Workflow & Quality Gates

- **Definition of Ready**: A feature cannot enter implementation until the code-first OpenAPI impact (controllers/DTOs to touch, generated `docs/contracts/openapi.yaml` preview or diff), prompt contract, and email template outline are documented, along with acceptance tests for success/failure paths.
...
**Version**: 2.4.0 | **Ratified**: 2025-11-22 | **Last Amended**: 2025-11-30
